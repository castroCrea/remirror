...
export class SchemaExtension extends PlainExtension {
  ...
  onCreate(): void {
    ...
    const { defaultBlockNode = 'paragraph' } = managerSettings;

    // True when the `defaultBlockNode` exists for this editor.
    const isValidDefaultBlockNode = this.store.tags[ExtensionTag.Block].includes(defaultBlockNode);
    ...
    const nodes: Record<string, NodeSpec> = isValidDefaultBlockNode
      ? {
          doc: object(),
          ...
          // it's position.
          [managerSettings.defaultBlockNode ?? 'paragraph']: object(),
        }
      : object();
