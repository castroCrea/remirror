...
import { command, CommandDecoratorOptions, CommandUiDecoratorOptions, helper } from './decorators';
...
export class CommandsExtension extends PlainExtension<CommandOptions> {
   ...
   */
  private readonly decorated = new Map<string, Remirror.CommandDecoratorOptions>();
  ...
  onView(view: EditorView<EditorSchema>): void {
    ...
    for (const extension of this.store.extensions) {
      ...
      for (const [commandName, options] of Object.entries(decoratedCommands)) {
        this.updateDecorated(commandName, options);
   ...
   */
  @helper()
  getCommandOptions(): Helper<Record<string, Remirror.CommandDecoratorOptions>> {
    const uiCommands: Record<string, Remirror.CommandDecoratorOptions> = {};
   ...
   * @internal
   */
  updateDecorated(name: string, options?: Remirror.CommandDecoratorOptions): void {
    if (!options) {
      this.decorated.delete(name);
      return;
    }

    const decoratorOptions = this.decorated.get(name) ?? {};
