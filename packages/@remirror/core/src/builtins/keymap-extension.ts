...
export class KeymapExtension extends PlainExtension<KeymapOptions> {
    ...
    for (const extension of this.store.extensions) {
      ...
      for (const [name, options] of entries(decoratedKeybindings)) {
        ...
        const shortcut = normalizeShortcut(
          isString(options.shortcut)
            ? options.shortcut
            : options.shortcut(extension.options, this.store),
          shortcutMap,
        );
        ...
        extensionKeymaps.push([priority, { [shortcut]: keyBinding }]);

        // Attach the normalized shortcut to the decorated command so that is
        // can be referenced in the UI.
        if (options.command) {
          commandsExtension.updateDecorated(options.command, { shortcut });
        }
      }
  ...
  arrowRight(parameter: KeyBindingParameter): boolean {
    const marks = this.store.markTags[ExtensionTag.SupportsExit];
...
function normalizeShortcut(value: string, shortcutMap: ShortcutMap) {
...
function updateNamedKeys(
  ...
  for (const [shortcutName, commandFunction] of entries(previousBindings)) {
    updatedBindings[normalizeShortcut(shortcutName, shortcutMap)] = commandFunction;
  }
